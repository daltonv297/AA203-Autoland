...
def scp_iteration(f, s0, s_goal, s_prev, u_prev, N, P, Q, R, T_max, delta_e_max, ρ):
    ...
    for k in range(N+1):
        constraints.append(cvx.norm(s_cvx[k]-s_prev[k]) <= ρ)
        if k == 0:
            constraints.append(s_cvx[k] == s0) # initial condition
        if k == N:
            cost_terms.append( cvx.quad_form(s_cvx[k]-s_goal,P) ) # state cost
            # constraints.append(s_cvx[k] == s_goal)
        else:
            cost_terms.append( cvx.quad_form(s_cvx[k]-s_goal,Q) ) # state cost
            cost_terms.append( cvx.quad_form(u_cvx[k],R) ) # control cost
            constraints.append(s_cvx[k+1] == A[k] @ s_cvx[k] + B[k] @ u_cvx[k] + c[k])
            constraints.append(cvx.norm(u_cvx[k]-u_prev[k]) <= ρ)
