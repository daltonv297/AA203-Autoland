...
def solve_scp(f, s0, s_goal, N, max_horizon, P, Q, R, T_max, delta_e_max, ρ, eps, max_iters, s_init=None, u_init=None):
    ...
    if s_init is None or u_init is None:
        s = np.zeros((max_horizon + 1, n))
        u = np.zeros((max_horizon, m))
        s[0] = s0
        for k in range(N):
            s[k+1] = f(s[k], u[k])
    else:
...
s_goal = np.array([150, 20, 0, np.deg2rad(5), 0, 0])  # desired state
Q = np.diag([10, 1e-3, 1e-1, 1e-1, 1e-5, 1])
P=Q
# P = np.diag([10, 10, 1e-1, 1e4, 1e-5, 100])
R = np.diag([5.0, 50.0])                 # control cost matrix

n = 6                                # state dimension
m = 2                                # control dimension
dt = 0.5                             # discrete time resolution
T = 15.                              # total simulation time
ρ = 0.5                               # trust region parameter
eps = 10#16e-2                           # convergence tolerance
N_scp = 40                      # maximum number of SCP iterations
horizon = 5                         # MPC horizon time
n_horizon = round(horizon / dt)        # MPC horizon steps
...
s_mpc = np.zeros((N, n_horizon + 1, n))
u_mpc = np.zeros((N, n_horizon, m))
u_out = np.zeros((N, m))
...
# s1, u1, = solve_scp(fd, s0, s_goal, n_horizon, P, Q, R, T_max, delta_e_max, ρ,
#                             eps, N_scp)
...
for t in tqdm(range(N)):
    ...
    # print(solve_airplane_landing_scp(f, s, s_goal, N, P, Q, R, eps, N_scp, s_init, u_init))
    # print(N-t)
    s_mpc[t], u_mpc[t] = solve_scp(fd, s0, s_goal, np.minimum(n_horizon, N-t), n_horizon, P, Q, R, T_max, delta_e_max, ρ, eps, N_scp, s_init, u_init)
